# за основу был взят font_dataset.py
# !! может изменить значение style_channel
import os
import torch
...
class ExtendFontDataset(BaseDataset):
    @staticmethod
    ...
    def __init__(self, opt):
        ...
        """
	    # <язык1>2<язык2>; 
	    # язык1 считается стилевым языком, откуда берем стиль шрифта
	    # язык2 - на какой язык шрифт переводим
        from_lan, to_lan = opt.direction.split('2')
        self.style_language = from_lan
        self.content_language = to_lan
		...
		# проверяем количество style-изображений,
		# - если строго меньше style_channel - меняем style_channel в опциях opt 
		# (для всех остальных пользователей opt это изменение будет доступно)
		# - если больше style_channel - оставляем style_channel без изменений
        style_images_path = os.path.join(opt.dataroot, opt.phase, self.style_language, opt.name)
        style_images_count = len(os.listdir(style_images_path))
        if style_images_count < opt.style_channel:
            opt.style_channel = style_images_count
